 ...
 * @extends {ActorSheet}
 */
export class plagueboundActorSheet extends ActorSheet {
  /** @override */
  ...
  /** @override */
  get template() {
    return `systems/ironbound/templates/actor/actor-${this.actor.type}-pb-sheet.hbs`;
  }
  ...
  activateListeners(html) {
    ...
    html.on("click", ".clickCorruption", this._handleCorruption.bind(this));
  ...
  _changeMoney(event) {
    ...
    const point = dataset.point;
    this.actor.update({
      [moneyName]: this.actor.system[money] + parseInt(point),
    });
  }
  ...
  _handleCorruption(event) {
    const element = event.currentTarget;
    const dataset = element.dataset;
    const mark = parseInt(dataset.mark);
    let corruption = this.actor.system.corruption + mark;
    this.actor.update({
